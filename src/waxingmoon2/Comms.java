...
//            System.out.println(bufferPool[13]);
    ...
    private static void getSpawnCenters() {
        ...
        spawnCenters[0] = spawnLocs[0].add(Direction.NORTHEAST);
        spawnCenters[1] = null;
        spawnCenters[2] = null;

        if (spawnCenters[1] == null && !spawnLocs[3].isAdjacentTo(spawnCenters[0])) {
            spawnCenters[1] = spawnLocs[3].add(Direction.NORTHEAST);
        } else if (!spawnLocs[3].isAdjacentTo(spawnCenters[0]) && !spawnLocs[3].isAdjacentTo(spawnCenters[1])) {
            spawnCenters[2] = spawnLocs[3].add(Direction.NORTHEAST);
            return;
        }
        if (spawnCenters[1] == null && !spawnLocs[6].isAdjacentTo(spawnCenters[0])) {
            spawnCenters[1] = spawnLocs[6].add(Direction.NORTHEAST);
        } else if (!spawnLocs[6].isAdjacentTo(spawnCenters[0]) && !spawnLocs[6].isAdjacentTo(spawnCenters[1])) {
            spawnCenters[2] = spawnLocs[6].add(Direction.NORTHEAST);
            return;
        }
        if (spawnCenters[1] == null && !spawnLocs[9].isAdjacentTo(spawnCenters[0])) {
            spawnCenters[1] = spawnLocs[9].add(Direction.NORTHEAST);
        } else if (!spawnLocs[9].isAdjacentTo(spawnCenters[0]) && !spawnLocs[9].isAdjacentTo(spawnCenters[1])) {
            spawnCenters[2] = spawnLocs[9].add(Direction.NORTHEAST);
            return;
        }
        if (spawnCenters[1] == null && !spawnLocs[12].isAdjacentTo(spawnCenters[0])) {
            spawnCenters[1] = spawnLocs[12].add(Direction.NORTHEAST);
        } else if (!spawnLocs[12].isAdjacentTo(spawnCenters[0]) && !spawnLocs[12].isAdjacentTo(spawnCenters[1])) {
            spawnCenters[2] = spawnLocs[12].add(Direction.NORTHEAST);
            return;
        }
        if (spawnCenters[1] == null && !spawnLocs[15].isAdjacentTo(spawnCenters[0])) {
            spawnCenters[1] = spawnLocs[15].add(Direction.NORTHEAST);
        } else if (!spawnLocs[15].isAdjacentTo(spawnCenters[0]) && !spawnLocs[15].isAdjacentTo(spawnCenters[1])) {
            spawnCenters[2] = spawnLocs[15].add(Direction.NORTHEAST);
            return;
        }
        if (spawnCenters[1] == null && !spawnLocs[18].isAdjacentTo(spawnCenters[0])) {
            spawnCenters[1] = spawnLocs[18].add(Direction.NORTHEAST);
        } else if (!spawnLocs[18].isAdjacentTo(spawnCenters[0]) && !spawnLocs[18].isAdjacentTo(spawnCenters[1])) {
            spawnCenters[2] = spawnLocs[18].add(Direction.NORTHEAST);
            return;
        }
        if (spawnCenters[1] == null && !spawnLocs[21].isAdjacentTo(spawnCenters[0])) {
            spawnCenters[1] = spawnLocs[21].add(Direction.NORTHEAST);
        } else if (!spawnLocs[21].isAdjacentTo(spawnCenters[0]) && !spawnLocs[21].isAdjacentTo(spawnCenters[1])) {
            spawnCenters[2] = spawnLocs[21].add(Direction.NORTHEAST);
            return;
        }
        if (spawnCenters[1] == null && !spawnLocs[24].isAdjacentTo(spawnCenters[0])) {
            spawnCenters[1] = spawnLocs[24].add(Direction.NORTHEAST);
        } else if (!spawnLocs[24].isAdjacentTo(spawnCenters[0]) && !spawnLocs[24].isAdjacentTo(spawnCenters[1])) {
            spawnCenters[2] = spawnLocs[24].add(Direction.NORTHEAST);
            return;
        }
    ...
    private static void writeAllyFlagID() throws GameActionException {
        if (Comms.myMoveOrder < 3) {
            writeToBufferPool(Comms.myMoveOrder, rc.senseNearbyFlags(8)[0].getID());
        }
    }
    private static void readAllyFlagID() {

    }
    private static void wipeAllyFlagID() {
