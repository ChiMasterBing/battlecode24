...
public class bugNav {
    ...
    public static MapLocation setTarget(MapLocation newTarget) {
        ...
        //if (rc.hasFlag()) System.out.println("set targ");
        MapLocation oldTarget = target;
        target = newTarget;
        return oldTarget;
    }

    public static MapLocation unsetTarget() {
        if (isBugging) endBugging();
        //if (rc.hasFlag()) System.out.println("unset targ");
        MapLocation oldTarget = target;
        target = null;
        return oldTarget;
    }
    ...
    public static void move(MapLocation loc) throws GameActionException {
        ...
        if (!rc.isMovementReady()) {
            rc.setIndicatorLine(location, loc, 0, 255, 255);
            if(isBugging) {
                bugNav.rc.setIndicatorLine(location, location.add(Utils.DIRS_CENTER[bugNavTracker.pivotDir]), 255, 0, 0);
                bugNav.rc.setIndicatorLine(location, location.add(Utils.DIRS_CENTER[bugNavTracker.pPivotDir]), 255, 255, 0);
                bugNav.rc.setIndicatorLine(location, location.add(Utils.DIRS_CENTER[bugNavTracker.ppPivotDir]), 0, 255, 0);
            }
            return;
        }
        ...
        if(isBugging) {
            bugNav.rc.setIndicatorLine(rc.getLocation(), rc.getLocation().add(Utils.DIRS_CENTER[bugNavTracker.pivotDir]), 255, 0, 0);
            bugNav.rc.setIndicatorLine(rc.getLocation(), rc.getLocation().add(Utils.DIRS_CENTER[bugNavTracker.pPivotDir]), 255, 255, 0);
            bugNav.rc.setIndicatorLine(rc.getLocation(), rc.getLocation().add(Utils.DIRS_CENTER[bugNavTracker.ppPivotDir]), 0, 255, 0);
        }
    ...
    public static Direction move(MapLocation loc, boolean flagHold) throws GameActionException {
        
        location = rc.getLocation();
        if (!rc.isMovementReady()) {
            rc.setIndicatorLine(location, loc, 0, 255, 255);
            bugNav.rc.setIndicatorLine(rc.getLocation(), bugNav.target, 0, 0, 255);
            if(isBugging) {
                bugNav.rc.setIndicatorLine(rc.getLocation(), rc.getLocation().add(Utils.DIRS_CENTER[bugNavTracker.pivotDir]), 255, 0, 0);
                bugNav.rc.setIndicatorLine(rc.getLocation(), rc.getLocation().add(Utils.DIRS_CENTER[bugNavTracker.pPivotDir]), 255, 255, 0);
                bugNav.rc.setIndicatorLine(rc.getLocation(), rc.getLocation().add(Utils.DIRS_CENTER[bugNavTracker.ppPivotDir]), 0, 255, 0);
        ...
        if (flagHold == true && withFlag == true && location.distanceSquaredTo(target) + 18 > location.distanceSquaredTo(loc)) {
        } else {
            withFlag = flagHold;
            if (!loc.equals(target)) {
                setTarget(loc);
    ...
    public static Direction navigate() throws GameActionException {
        ...
        //! radius bugNav
        // if (isBugging && (location.distanceSquaredTo(target) < bugNavTracker.orginalDist)) {
        //     endBugging();
        // }
        //! line bugNav
        if (isBugging && location.distanceSquaredTo(target) < bugNavTracker.orginalDist && bugNavTracker.originalPos.directionTo(location).equals(bugNavTracker.originalDir)) {
            if (rc.hasFlag()) System.out.print("line condition");
            endBugging();
        }
        if (isBugging && (Utils.rng.nextInt(25) < 1))
            endBugging();

        //! try greedy moves
        if (!isBugging || bugNavTracker.pivotDir == 8) {
            if (isBugging) endBugging();
            //if (rc.hasFlag())  System.out.println("pivot 8");
    ...
    public static void makeMove(Direction dir) {
        ...
        if (Utils.dxDyToInt(dir) != bugNavTracker.nextPivot()) {
            //if (rc.hasFlag())  System.out.println("makemove1 " + String.valueOf(Utils.dxDyToInt(dir)) + " " + String.valueOf(bugNavTracker.nextPivot()));
            endBugging();
        }
        else if (bugNavTracker.moveToNextPivot()) {
            //if (rc.hasFlag())  System.out.println("makemove22");
            endBugging();
        }
    }
    ...
    private static int getBugMove() throws GameActionException {
        ...
        if (dirStatus[bugNavTracker.pivotDir] == OPEN || dirStatus[bugNavTracker.pivotDir] == WATER) {
            ...
            if (bugNavTracker.revertPivot()) {
                //if (rc.hasFlag()) System.out.println("rfeverting");
                endBugging();
                return 8;
            }
            return dirToMove;
        ...
        while (dirToMove != originalPivotDir) {
            if (bugNavTracker.passed.contains((Utils.locationToInt(location) <<3) | bugNavTracker.pivotDir)) {
               // if (rc.hasFlag()) System.out.println("passed");
                endBugging();
                return 8;
            }
            switch (dirStatus[dirToMove]) {
                ...
                case BORDER:
                    ...
                    if (bugNavTracker.reverse == true) {
                        //if (rc.hasFlag())  System.out.println("wborder2");
                        endBugging();
                        return 8;
                    } 
    ...
    private static int genDirStatus(int dirInt) {
        if (dirMapInfos[dirInt] == null) {
            return (!isBugging || !bugNavTracker.reverse)?WALL:WALL;
    ...
    private static int rotateDir(int dirInt, boolean reverse) {
        return reverse ? (dirInt - ROTATE + 8) % 8 : (dirInt + ROTATE + 8) % 8;    
    }
}
...
class BugNavTracker {
    ...
    public boolean moveToNextPivot() {
        ...
        //if (bugNav.rc.hasFlag()) System.out.println(String.valueOf(newPivotDx) + " " + String.valueOf(newPivotDy));
