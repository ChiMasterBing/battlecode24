        ...
        if (rc.hasFlag()){
            for (Direction d:Direction.allDirections()) {
                if (spawnSet.contains(myLoc.add(d)) && rc.canMove(d)) {
                    rc.move(d);
                    if (bugNav.isBugging) bugNav.endBugging();
            ...
            MapLocation tempObstacle = bugNav.isBugging ? myLoc.add(Utils.DIRECTIONS[bugNav.bugNavTracker.pivotDir]) : null;
...
//                BFSController.move(rc, currentTarget);
    ...
    public void movement() throws GameActionException {
        ...
        for(RobotInfo i: closeFriendlyRobots){
            if(i.getLocation().distanceSquaredTo(myLoc)<2){
                ...
                if(ret>2) {
                    ...
                    bugNav2.move(myLoc.add(oppDir).add(oppDir));
                    return;
                }
            }
        }
        ...
        if(lowestHealthLoc!=null&&(rc.getLevel(SkillType.HEAL)>3||rc.getID()%3<2)){
            bugNav2.move(lowestHealthLoc);//I cant bfs cuz osmeimtes it moves perpenduclarly into enemy base
        }
