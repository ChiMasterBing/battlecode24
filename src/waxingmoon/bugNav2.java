...
class BugNavTracker {
    ...
    public BugNavTracker(int pivotDirInt) {
        ...
        orginalDist = bugNav2.location.distanceSquaredTo(bugNav2.target);
        originalPos = bugNav2.location;
    ...
    public int nextPivot() {
        return reverse ? (pivotDir - bugNav2.ROTATE + 8) % 8 : (pivotDir + bugNav2.ROTATE + 8) % 8;
    }

    //! returns true if fails
    public boolean moveToNextPivot() {
        
        passed.add((Utils.locationToInt(bugNav2.rc.getLocation().add(Utils.DIRS_CENTER[nextPivot()].opposite())) << 3) | pivotDir);
