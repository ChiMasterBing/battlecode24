...
public class Attacker extends Robot {
    ...
    boolean previouslyStunned = false;
...
//        }
    ...
    public boolean shouldIHeal(boolean second) {
        if (rc.getLevel(SkillType.HEAL) > 3) return true;
        if (rc.getLevel(SkillType.ATTACK) > 3) return false; 
        for (RobotInfo r:closeFriendlyRobots) {
            if (r.getAttackLevel() > 3) return true;
        }
        if (second && (closestEnemy == null || myLoc.distanceSquaredTo(closestEnemy) > 9)) return true;
        return false;
    }
...
//                    System.out.println(best.ID);
    ...
    public Boolean crumbMovementLogic() throws GameActionException {
        ...
        if (crummy.length > 0 && roundNumber < 250) { //crummy.length > (numberOfFriendlies+1) //I CHANGED THIS
...
//            }
        ...
        AttackerMicro.setGlobals(enemyRobots, friendlyRobots, closeEnemyRobots, closeFriendlyRobots, myMoveNumber);
    }
    ...
    public void tryHeal() throws GameActionException {
        ...
        for(RobotInfo i: closeFriendlyRobots){
            ...
            if (i.getHealLevel() >= 3) {
                score += 100;
            } else if (i.getAttackLevel() >= 3) {
                score += 250;
            }
            // score += (i.getHealLevel() + 5*i.getAttackLevel()) * 7;
            if (score > bestScore) {
                bestScore = score;
...
//            rc.setIndicatorString("I healed " + bestHeal);
    ...
    public void attackLogic() throws GameActionException {
        ...
        MapLocation attackLoc = AttackerMicro.findBestAttackLocation();
    ...
    public boolean attackMicro() throws GameActionException {
        ...
        return AttackerMicro.movementMicro();
    }
